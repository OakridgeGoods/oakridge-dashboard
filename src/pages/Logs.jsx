...
export default function LogsPage({ token }) {
  ...
  const filtered = logs.filter(l => {
    ...
    if (search) {
      ...
      if (![l.Script, l.Function, l.Status, l.SKU, l.Details].some(f => (f || '').toLowerCase().includes(q))) return false
    }
    return true
  })
  ...
  const selectStyle = {
    border: '0.5px solid var(--border-md)', borderRadius: 6, padding: '6px 10px',
    ...
    <div style={{ display: 'flex', gap: 20, height: 'calc(100vh - 52px - 48px)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 600, marginRight: 4, flexShrink: 0 }}>Period:</span>
          {DATE_PRESETS.map(p => (
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            style={{ border: '0.5px solid var(--border-md)', borderRadius: 7, padding: '6px 12px', fontSize: 13, background: 'var(--surface)', color: 'var(--text)', outline: 'none', width: 280 }}
          />
          <select value={scriptFilter} onChange={e => setScriptFilter(e.target.value)} style={selectStyle}>
            {scripts.map(s => <option key={s} value={s}>{s === 'All' ? 'All Scripts' : s}</option>)}
          </select>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={selectStyle}>
            {statuses.map(s => <option key={s} value={s}>{s === 'All' ? 'All Statuses' : s}</option>)}
          </select>
          {(scriptFilter !== 'All' || statusFilter !== 'All' || search) && (
            <button onClick={() => { setScriptFilter('All'); setStatusFilter('All'); setSearch('') }}
              style={{ fontSize: 11, color: 'var(--text-3)', background: 'none', border: '0.5px solid var(--border)', borderRadius: 5, padding: '4px 8px', cursor: 'pointer' }}>
              ✕ Clear
            </button>
          )}
          <span style={{ fontSize: 11, color: 'var(--text-3)', marginLeft: 'auto' }}>{filtered.length} entries</span>
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 12, overflow: 'hidden', flex: 1, overflowY: 'auto' }}>
          {loading ? <Spinner /> : filtered.length === 0 ? <EmptyState message="No log entries for this period" /> : (
            <Table headers={['Timestamp', 'Script', 'Function', 'Status', 'SKU', 'Details']}>
              {filtered.map((l, i) => (
                <TR key={i}>
                  <TD><Mono size={11} style={{ color: 'var(--text-3)', whiteSpace: 'nowrap' }}>{l.Timestamp}</Mono></TD>
                  <TD><span style={{ fontSize: 11, fontWeight: 600, fontFamily: 'var(--font-mono)', whiteSpace: 'nowrap' }}>{l.Script || '—'}</span></TD>
                  <TD><Mono size={11}>{l.Function || '—'}</Mono></TD>
                  <TD><Badge type={statusBadgeType(l.Status)}>{l.Status || '—'}</Badge></TD>
                  <TD><Mono size={11}>{l.SKU || '—'}</Mono></TD>
                  <TD style={{ fontSize: 11.5, color: 'var(--text-2)', maxWidth: 380, wordBreak: 'break-word' }}>{l.Details || ''}</TD>
                </TR>
              ))}
            </Table>
          )}
      ...
      <div style={{ width: 260, flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 10, marginTop: 2 }}>
          Most Recent
        </div>
        <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 12, overflow: 'hidden', flex: 1, overflowY: 'auto' }}>
          {loading ? <Spinner /> : recent.length === 0 ? (
            <div style={{ padding: 20, fontSize: 12, color: 'var(--text-3)', textAlign: 'center' }}>No entries</div>
          ) : (
            recent.map((l, i) => (
              <div key={i} style={{
                padding: '10px 14px',
                ...
                <div style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--text)', fontFamily: 'var(--font-mono)', marginBottom: 2 }}>
                  {l.Script || '—'}
                </div>
